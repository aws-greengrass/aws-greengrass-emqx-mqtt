...
-export([get_ssl_listener/1]).
-export([get_opts/1, get_ssl_opts/1, apply_opts/2, apply_ssl_opts/2]).
-export([set_custom_verify_fun/2]).
-export([restart_listener/1]).


%% Retrieve the EMQX SSL listener by name.
-spec(get_ssl_listener(string()) -> emqx_listeners:listener() | false).
get_ssl_listener(Name) ->
  emqx_listeners:find_by_id(emqx_listeners:identifier(#{proto => ssl, name => list_to_binary(Name)})).
